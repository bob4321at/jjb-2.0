...
func flyingEnemyMovement(enemy *Enemy, player_pos utils.Vec2, level_hitbox []utils.HitBox, speed float64, height_above_ground float64) {
	...
	if utils.Collide(utils.Vec2{X: enemy.Pos.X, Y: enemy.Pos.Y + enemy.Vel.Y + 2}, utils.Vec2{X: float64(enemy.Tex.GetTexture().Bounds().Dx()), Y: float64(enemy.Tex.GetTexture().Bounds().Dy()) + height_above_ground}, utils.Vec2{X: 2000 - (1280 / 2), Y: -2000 - (720 / 2) - (575/2 - 32)}, utils.Vec2{X: 2048, Y: (126 * 2)}) {
		enemy.Vel.Y -= 1
	}
