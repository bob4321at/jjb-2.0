...
func cloudHeadUpdate(enemy *Enemy, player_pos utils.Vec2, level_hitbox []utils.HitBox) {
	...
	for le := 0; le < len(Enemies_In_World); le++ {
		...
		if enemy != oe {
			if utils.Collide(utils.Vec2{X: enemy.Pos.X, Y: enemy.Pos.Y + enemy.Vel.Y}, utils.Vec2{X: 32, Y: 32}, utils.Vec2{X: float64(oe.Pos.X), Y: float64(oe.Pos.Y)}, utils.Vec2{X: float64(oe.Tex.GetTexture().Bounds().Dx()), Y: float64(oe.Tex.GetTexture().Bounds().Dy())}) {
				enemy.Vel.Y = -enemy.Vel.Y / 1.2
				oe.Vel.Y = enemy.Vel.Y / 1.2
			}
			if utils.Collide(utils.Vec2{X: enemy.Pos.X + enemy.Vel.X, Y: enemy.Pos.Y}, utils.Vec2{X: 32, Y: 32}, utils.Vec2{X: float64(oe.Pos.X), Y: float64(oe.Pos.Y)}, utils.Vec2{X: float64(oe.Tex.GetTexture().Bounds().Dx()), Y: float64(oe.Tex.GetTexture().Bounds().Dy())}) {
	...
	for ti := 0; ti < len(level_hitbox); ti++ {
		...
		if utils.Collide(utils.Vec2{X: enemy.Pos.X, Y: enemy.Pos.Y + enemy.Vel.Y}, utils.Vec2{X: 32, Y: 128}, utils.Vec2{X: t.X, Y: t.Y}, utils.Vec2{X: 32, Y: 32}) {
			enemy.Vel.Y = 0
		}
